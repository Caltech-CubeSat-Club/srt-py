...
{#snippet el_ring(el_deg: number, ringRenderOrder: number)}
    ...
    <T.Mesh position={[0, RADIUS * Math.sin(elRad), 0]} rotation={[PI / 2, 0, 0]} renderOrder={ringRenderOrder}>
        <T.TorusGeometry args={[RADIUS * Math.cos(elRad), .75 / uFovScale.value, 8, 64]} />
        <T.ShaderMaterial
...
{#snippet az_ring(az_deg: number, color: string, ringRenderOrder: number)}
    ...
    <T.Mesh position={[0, 0, 0]} rotation={[0, -az, PI / 2]} renderOrder={ringRenderOrder}>
