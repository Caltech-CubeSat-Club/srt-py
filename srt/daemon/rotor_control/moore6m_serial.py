...
Serial-backed controller for the Moore/Caltech 6m rotor.
"""
...
class Moore6mSerial(Motor):
    ...
    def __init__(self, port="COM1", baudrate=115200, az_limits=(-89, 449), el_limits=(15, 81), verbose=False, safe_mode=False):
        ...
        self._transition_state(Moore6mSerial.State.CONNECTING, "opening serial port")
        ...
        try:
            if self.safe_mode:
                logging.warning(
                    "Moore6m safe mode enabled: status-only (motion/control commands blocked)"
                )
                self._transition_state(
                    Moore6mSerial.State.READY, "safe mode status-only startup"
                )
            else:
                self.startup()
            self.get_info()
    ...
    def _set_fault(self, message):
        ...
        self._transition_state(Moore6mSerial.State.FAULT, message)
    ...
    def _verify_response(self, command_text, response, expected_prefix=None):
        ...
        ack_prefix = Moore6mSerial.ACK_PREFIX_BY_COMMAND.get(token)
    ...
    def _send_command_direct(self, cmd, command_text, send_delay, recv_delay, expected_prefix, retries, fail_on_error):
        last_response = ""
    ...
    def startup(self):
        self._transition_state(Moore6mSerial.State.STARTUP_SYNC, "running startup sequence")
        for _ in range(8):
            self.serial.write(b"\r")
            sleep(0.003)
        ...
        for attempt in range(self.startup_retries + 1):
            ...
            if startup_ok:
                break
            self._transition_state(
                Moore6mSerial.State.RECOVERING,
                f"startup retry {attempt + 1}/{self.startup_retries + 1}",
            )
            sleep(0.1)
        if not startup_ok:
            self._set_fault("startup sequence failed")
            raise RuntimeError("Moore6m startup sequence failed")
        self._transition_state(Moore6mSerial.State.READY, "startup sequence complete")
        logging.warning("startup sequence complete")

    def send_command(self, cmd, send_delay=0.008, recv_delay=0.008, expected_prefix=None, retries=None, fail_on_error=True):
        if retries is None:
            retries = self.max_command_retries
        ...
        if self.safe_mode and command_token not in Moore6mSerial.READ_ONLY_COMMANDS:
    ...
    def point(self, az, el):
        ...
        if az < self.az_limits[0] or az > self.az_limits[1]:
            logging.error(
                "requested azimuth %.3f is outside limits [%s, %s]",
                az,
                self.az_limits[0],
                self.az_limits[1],
            )
        ...
        if el < self.el_limits[0]:
            logging.error(
                "requested location is below horizon (min %.3f deg elevation)",
                self.el_limits[0],
            )
            return
        if el > self.el_limits[1]:
            logging.error(
                "requested object is higher than zenith (max %.3f deg elevation)",
                self.el_limits[1],
            )
            return
        self.brakes_off()
    ...
    def calibrate(self):
        ...
        self._transition_state(Moore6mSerial.State.CALIBRATING, "calibration started")
        for cmd, pattern in seq:
            resp = self.send_command(
                cmd,
                send_delay=0.008,
